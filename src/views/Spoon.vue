	...
	<div class="row ma-0" style="height: 100vh; overflow-y: hidden;">
		...
		<modal :show.sync="modal.block">
            ...
            <template slot="footer">
                <base-button type="warning" class="ml-auto" @click="blockUser(selectedUser); modal.block = false; modal.profile = false;">OK</base-button>
...
export default {
	...
	mounted() {
		...
		if ( app.get('spoon.filter') ) {
			...
			/*
			this.$s(app.get('user.token')).getLive()
				.then(res => {
					this.liveList = res;
				});
			*/
